...
class Seq2SeqModel:
    ...
    def multilabel_categorical_crossentropy(y_pred, y_true):
        ...
        return (neg_loss + pos_loss).mean() * 10
                ...
                if args.fp16:
                    with amp.autocast():
                        ...
                        if self.regularization:
                            ...
                            #     cross_attention = torch.cat((outputs['cross_attentions'][i][:,:,:16,1:2],outputs['cross_attentions'][i][:,:,:16,5:6]),3)
                            #     cross_attention = torch.transpose(input=cross_attention, dim0=2, dim1=3)
                            #     x = torch.nn.functional.normalize(cross_attention, p=2, dim=3)
                            #     y = torch.transpose(input=x, dim0=2, dim1=3)
                            #     e = torch.eye(2,2)[None,None,:,:].repeat(cross_attention.size(0),cross_attention.size(1), 1, 1).to(self.device)
                            #     regularization_loss = torch.norm(torch.matmul(x, y) - e)

                            #     loss += regularization_loss/(cross_attention.size(0))
                            
                            labels = []
                            for label in inputs["labels"]:
                                flag = 0
                                for id in label[:32]:
                                    if id in [34, 117, 9355, 7]:
                                        flag += 1
                                if flag >= 4:
                                    labels.append(1)
                                else:
                            ...
                            # encoder_attentions = torch.cat((outputs['encoder_attentions'][i][:,:,9:,1:2],outputs['encoder_attentions'][i][:,:,9:,5:6]),3)
                            # encoder_attentions = torch.transpose(input=encoder_attentions, dim0=2, dim1=3)
                            # x = torch.nn.functional.normalize(encoder_attentions, p=2, dim=3)
                            # y = torch.transpose(input=x, dim0=2, dim1=3)
                            # e = torch.eye(2,2)[None,None,:,:].repeat(encoder_attentions.size(0),encoder_attentions.size(1), 1, 1).to(self.device)
                            # regularization_loss = torch.norm(torch.matmul(x, y) - e)
                            # loss += regularization_loss//len(labels)# (encoder_attentions.size(0))  
                            i = 0
                            for j in range(len(labels)):
                                word_num = torch.nonzero(inputs['decoder_input_ids'][j]==2)[0]
                                if labels[j]==0:
                                    att_pos=outputs['cross_attentions'][i][j,:,1:word_num,5]
                                    att_neg=outputs['cross_attentions'][i][j,:,1:word_num,1]
                                else:
                                    att_pos=outputs['cross_attentions'][i][j,:,1:word_num,1]
                                    att_neg=outputs['cross_attentions'][i][j,:,1:word_num,5]
                                    
                                # pos_loss = torch.logsumexp(-att_pos, dim=1)
                                # neg_loss = torch.logsumexp(att_neg, dim=1)

                                # boundary = torch.tensor([0.01]).repeat(att_pos.shape[0]).to(self.device)        
                                # pos_loss = torch.log(torch.sum(torch.exp(-att_pos), dim=1)+torch.exp(-boundary))
                                # neg_loss = torch.log(torch.sum(torch.exp(att_neg), dim=1)+torch.exp(boundary))

                                boundary = torch.tensor([1]).repeat(att_pos.shape[0]).to(self.device)        
                                pos_loss = torch.log(torch.sum(torch.exp(-att_pos/0.01), dim=1)+torch.exp(-boundary))
                                neg_loss = torch.log(torch.sum(torch.exp(att_neg/0.01), dim=1)+torch.exp(boundary))

                                loss+=((neg_loss+pos_loss).mean())/len(labels)
                            # i = 0
                            # for j in range(len(labels)):
                                
                            #     if labels[j]==0:
                            #         cross_attention = outputs['cross_attentions'][i][j,:,:16,5:6]
                            #     else:
                            #         cross_attention = outputs['cross_attentions'][i][j,:,:16,1:2]
                            #     loss+=torch.norm(cross_attention)/len(labels)
                else:
                    outputs = model(**inputs)
                    # model outputs are always tuple in pytorch-transformers (see doc)
                    loss = outputs[0]
